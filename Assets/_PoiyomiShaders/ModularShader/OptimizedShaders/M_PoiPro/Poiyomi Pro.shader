Shader "Hidden/Locked/.poiyomi/V8.0/Pro/267e1b316da597347bc70350979e0e7d"
{
	Properties
	{
		...
		[Space(8)]
		[ThryTexture]_MochieMetallicMap ("Metallic Map (B)--{reference_properties:[_MochieMetallicMapPan, _MochieMetallicMapUV, _MochieMetallicMapInvert]}", 2D) = "white" { }
		[HideInInspector][Vector2]_MochieMetallicMapPan ("Panning", Vector) = (0, 0, 0, 0)
		[HideInInspector][ToggleUI]_MochieMetallicMapInvert ("Invert", Float) = 0
		...
		[ThryTexture]_MochieRoughnessMap ("Roughness Map (G)--{reference_properties:[_MochieRoughnessMapPan, _MochieRoughnessMapUV, _MochieRoughnessMapInvert]}", 2D) = "white" { }
		[HideInInspector][Vector2]_MochieRoughnessMapPan ("Panning", Vector) = (0, 0, 0, 0)
		[HideInInspector][ToggleUI]_MochieRoughnessMapInvert ("Invert", Float) = 0
		...
		[ThryTexture]_MochieReflectionMask ("Reflection Mask (R)--{reference_properties:[_MochieReflectionMaskPan, _MochieReflectionMaskUV, _MochieReflectionMaskInvert]}", 2D) = "white" { }
		[HideInInspector][Vector2]_MochieReflectionMaskPan ("Panning", Vector) = (0, 0, 0, 0)
		[HideInInspector][ToggleUI]_MochieReflectionMaskInvert ("Invert", Float) = 0
		...
		[ThryTexture]_MochieSpecularMask ("Specular Mask (R)--{reference_properties:[_MochieSpecularMaskPan, _MochieSpecularMaskUV, _MochieSpecularMaskInvert]}", 2D) = "white" { }
		[HideInInspector][Vector2]_MochieSpecularMaskPan ("Panning", Vector) = (0, 0, 0, 0)
		[HideInInspector][ToggleUI]_MochieSpecularMaskInvert ("Invert", Float) = 0
...
#define POI_MATCAP0
#define POI_MATCAP0_CUSTOM_NORMAL
...
#define PROP_DECALOVERIDEALPHA 0
...
#define PROP_DECALOVERIDEALPHA1 0
...
#define PROP_DECALOVERIDEALPHA2 0
...
#define PROP_DECALOVERIDEALPHA3 0
...
#define PROP_LIGHTINGCOLORMODE 1
...
#define PROP_LIGHTINGMINLIGHTINDIBRIGHTNESS 0
...
#define PROP_LIGHTINGGRADIENTSTARTWRAP 0
#define PROP_LIGHTINGGRADIENTENDWRAP 0.5
...
#define PROP_CLOTHLERP 0
#define PROP_CLOTHMETALLIC 0
...
#define PROP_MATCAP0CUSTOMNORMAL 1
#define PROP_MATCAP0NORMALMAP
...
#define PROP_MOCHIEMETALLICMAPUV 0
#define PROP_MOCHIEROUGHNESSMAPINVERT 0
#define PROP_MOCHIEROUGHNESSMAPUV 0
#define PROP_MOCHIEREFLECTIONMASKINVERT 0
#define PROP_MOCHIEREFLECTIONMASKUV 0
#define PROP_MOCHIESPECULARMASKINVERT 0
#define PROP_MOCHIESPECULARMASKUV 0
...
#define PROP_CLEARCOATNORMAL 0
#define PROP_CLEARCOATTINTTHEMEINDEX 0
#define PROP_CLEARCOAT 1
#define PROP_CLEARCOATSMOOTHNESS 0
#define PROP_CLEARCOATSAMPLEWORLD 1
...
#define PROPM_SPECIALFXCATEGORY 1
...
#define PROP_FLIPBOOKCHRONOTENSITYSPEED 1
...
#define PROP_EMISSIONSTRENGTH 1
...
#define PROP_AUDIOLINKEMISSION0CENTEROUTWIDTH 1
#define PROP_AUDIOLINKEMISSION0CENTEROUTSIZE 1
...
#define PROP_EMISSIONREPLACE1 1
...
#define PROP_EMISSIONSTRENGTH1 1
...
#define PROP_AUDIOLINKEMISSION1CENTEROUTWIDTH 1
#define PROP_AUDIOLINKEMISSION1CENTEROUTSIZE 1
...
#define PROP_EMISSIONREPLACE2 1
...
#define PROP_EMISSIONSTRENGTH2 1
...
#define PROP_AUDIOLINKEMISSION2CENTEROUTWIDTH 1
#define PROP_AUDIOLINKEMISSION2CENTEROUTSIZE 1
...
#define PROP_EMISSIONREPLACE3 1
...
#define PROP_EMISSIONSTRENGTH3 1
...
#define PROP_AUDIOLINKEMISSION3CENTEROUTWIDTH 1
#define PROP_AUDIOLINKEMISSION3CENTEROUTSIZE 1
...
#define PROP_DEPTHALPHAMINVALUE 0
#define PROP_DEPTHALPHAMAXVALUE 1
...
#define PROP_TEXTGLYPHS
#define PROP_TEXTPIXELRANGE 4
#define PROP_TEXTENABLED 0
#define PROPM_START_TEXTFPS 1
#define PROP_TEXTFPSENABLED 1
...
#define PROP_TEXTPOSITIONENABLED 1
...
#define PROP_DISTORTIONSTRENGTH 0.03
#define PROP_DISTORTIONSTRENGTH1 0.01
...
#define PROPM_START_PARALLAX 1
#define PROP_POIPARALLAX 1
#define PROP_PARALLAXUV 0
#define PROP_HEIGHTMAPUV 0
#define PROP_HEIGHTSTRENGTH 0.1
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			{
				float2 uvs = poiUV(poiMesh.uv[float(0)], float4(1,1,0,0));
				float2 dx = ddx(uvs);
				float2 dy = ddy(uvs);
				float3 result = 0;
				int stepIndex = 0;
				...
				while (stepIndex < numSteps + 1)
				{
					...
					else
					{
						...
						currRayZ -= layerHeight * (1 - result.z) * (1 + float(1));
					}
				}
				int sectionSteps = 10;
				int sectionIndex = 0;
			...
			void applyParallax(inout PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam)
			{
				...
				poiMesh.uv[float(0)] = POM(poiLight, _HeightMap, poiMesh, poiCam.viewDir, poiCam.tangentViewDir, float(10), float(128), float(0.1), 0, float4(1,1,0,0).xy, float2(float(0), float(0)));
				#else
				float2 offset = POM(poiLight, _HeightMap, poiMesh, poiCam.viewDir, poiCam.tangentViewDir, float(10), float(128), float(0.1), 0, float4(1,1,0,0).xy, float2(float(0), float(0)));
				if (float(0) == 0)       poiMesh.uv[0] = offset;
				if (float(0) == 1)       poiMesh.uv[1] = offset;
				if (float(0) == 2)       poiMesh.uv[2] = offset;
				if (float(0) == 3)       poiMesh.uv[3] = offset;
				if (float(0) == 4)       poiMesh.uv[4] = offset;
				if (float(0) == 5)       poiMesh.uv[5] = offset;
				if (float(0) == 6)       poiMesh.uv[6] = offset;
				if (float(0) == 7)       poiMesh.uv[7] = offset;
				#endif
			}
			#endif
			void applyAlphaOptions(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					...
					half3 modelForwardDirection = normalize(mul(unity_ObjectToWorld, normalize(float4(0,0,1,1))));
			...
			void applyDithering(inout PoiFragData poiFragData, in PoiCam poiCam)
			{
				if (float(0))
				{
					poiFragData.alpha = poiFragData.alpha - (calcDither(poiCam.screenUV) * (1 - poiFragData.alpha) * float(0.1));
				}
			}
			void ApplyAlphaToCoverage(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if (float(0) == 1)
				{
					...
					if (float(0))
					{
						poiFragData.alpha *= 1 + max(0, CalcMipLevel(poiMesh.uv[0] * float4(0.25,0.25,4,4).zw)) * float(0.25);
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.rampedLightMap = saturate((poiLight.lightMap - float(0)) / saturate(float(0.5) - float(0) + fwidth(poiLight.lightMap)));
				poiLight.finalLighting = lerp((float4(1,1,1,1) * lerp(poiLight.indirectColor, poiLight.directColor, float(0)) * poiLight.occlusion), (poiLight.directColor), saturate(poiLight.rampedLightMap + 1 - shadowStrength));
				#endif
				...
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				poiLight.finalLighting = UNITY_BRDF_PBS(1, 0, 0, float(0), poiMesh.normals[1], poiCam.viewDir, light, CreateIndirectLight(poiMesh, poiCam, poiLight)).xyz;
				#endif
				#ifdef _LIGHTINGMODE_CLOTH
				...
				float metallic =  pow(clothmapsample.r *float(0), 2) * ClothMetallic(clothmask);
				roughness = float(0) == 1 ? 1 - roughness : roughness;
				#else
				float roughness = 1 - (float(0.5));
				float metallic = pow(float(0), 2);
				...
				poiLight.rampedLightMap = saturate((poiLight.lightMap - float(0)) / saturate(float(0.5) - float(0) + fwidth(poiLight.lightMap)));
				poiLight.rampedLightMap = lerp(float4(1,1,1,1) * lerp(poiLight.indirectColor, 1, float(0)), float3(1, 1, 1), saturate(1 - smoothstep(float(0) - .000001, float(0.5), 1 - poiLight.lightMap)));
				poiLight.rampedLightMap = lerp(float3(1, 1, 1), poiLight.rampedLightMap, shadowStrength.r);
				poiLight.finalLighting = (poiLight.indirectColor + poiLight.directColor) * saturate(poiLight.rampedLightMap + 1 - float(1));
				#endif
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(1) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.5) * poiLight.vAttenuation[index], smoothstep(float(0), float(0.5), poiLight.vDotNL[index])) * poiLight.detailShadow;
					}
					if (float(1) == 2) //if(float(1) == 2) // Wrapped
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.5) * poiLight.vAttenuation[index], smoothstep(float(0), float(0.5), poiLight.vDotNL[index])) * poiLight.detailShadow;
					}
				}
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float matcap2Mask = 0;
				float2 matcapUV = 0;
				float3 normal0 = poiMesh.normals[float(1)];
				#ifdef POI_MATCAP0
				#ifdef POI_MATCAP0_CUSTOM_NORMAL
				#if defined(PROP_MATCAP0NORMALMAP) || !defined(OPTIMIZER_ENABLED)
				normal0 = calculateNormal(poiMesh.normals[float(1)], poiMesh, _Matcap0NormalMap, float4(3,3,0,0), float4(0,0,0,0), float(0), float(1));
				#endif
				#endif
				switch(float(1))
				{
					case 0:
					{
						...
						matcapUV = noSknewViewNormal.rg * float(0.43) + 0.5;
						break;
					}
					case 1:
					{
						...
						matcapUV = float2(dot(worldViewRight, normal0), dot(worldViewUp, normal0)) * float(0.43) + 0.5;
						break;
					}
					case 2:
					{
				...
				#if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
				matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#else
				matcap = float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#endif
				matcap.rgb *= float(1);
				#if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
				matcapMask = POI2D_SAMPLER_PAN(_MatcapMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				matcapMask = 1;
				#endif
				if (float(0))
				...
				poiFragData.alpha *= lerp(1, matcap.a, matcapMask * float(0));
				if (float(0))
				{
					matcap.rgb = hueShift(matcap.rgb, float(0) + _Time.x * float(0));
				}
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
				...
				#endif
				#ifdef COLOR_GRADING_HDR_3D
				float3 normal1 = poiMesh.normals[float(1)];
				#ifdef POI_MATCAP1_CUSTOM_NORMAL
				#if defined(PROP_MATCAP1NORMALMAP) || !defined(OPTIMIZER_ENABLED)
				normal1 = calculateNormal(poiMesh.normals[float(1)], poiMesh, _Matcap1NormalMap, float4(3,3,0,0), float4(0,0,0,0), float(0), float(1));
				#endif
				#endif
				matcapUV = 0;
				switch(float(1))
				{
					...
					case 1:
					{
						float3 worldViewUp = normalize(float3(0, 1, 0) - poiCam.viewDir * dot(poiCam.viewDir, float3(0, 1, 0)));
						float3 worldViewRight = normalize(cross(poiCam.viewDir, worldViewUp));
						matcapUV = float2(dot(worldViewRight, normal1), dot(worldViewUp, normal1)) * float(0.43) + 0.5;
						break;
					}
					case 2:
					{
				...
				#if defined(PROP_MATCAP2) || !defined(OPTIMIZER_ENABLED)
				matcap2 = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap2, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap2)) * float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#else
				matcap2 = float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#endif
				matcap2.rgb *= float(1);
				#if defined(PROP_MATCAP2MASK) || !defined(OPTIMIZER_ENABLED)
				matcap2Mask = POI2D_SAMPLER_PAN(_Matcap2Mask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				matcap2Mask = 1;
				#endif
				if (float(0))
				...
				poiFragData.alpha *= lerp(1, matcap2.a, matcap2Mask * float(0));
				if (float(0))
				{
					matcap2.rgb = hueShift(matcap2.rgb, float(0) + _Time.x * float(0));
				}
				blendMatcap(poiLight, poiFragData, float(1), float(0), float(0), matcap2, matcap2Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				#endif
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(poiMesh.uv[float(0)].xy, float4(10,10,0,0)) + _Time.x * float4(0,0,0,0));
				...
				#endif
				if (float(1) == 0) // Poi Custom Light Color
				{
				...
				if (float(1) == 1) // More standard approach to light color
				{
